...
{   
	...
	for (i = 0; i < src_len - 3; i += 3)
	{
		...
		register int b0 = GetB64Char((base64_B0(ulTmp) >> 2) & 0x3F);
		register int b1 = GetB64Char((base64_B0(ulTmp) << 6 >> 2 | B1(ulTmp) >> 4) & 0x3F);
	...
	if (i < src_len)
	{
		...
		p64[0] = GetB64Char((base64_B0(ulTmp) >> 2) & 0x3F);
		p64[1] = GetB64Char((base64_B0(ulTmp) << 6 >> 2 | B1(ulTmp) >> 4) & 0x3F);
...
{   
	...
	for (i = 0; i < src_len - 4; i += 4)
	{
		...
		register int b0 = (GetB64Index((char)base64_B0(ulTmp)) << 2 | GetB64Index((char)B1(ulTmp)) << 2 >> 6) & 0xFF;
