...
public:
	...
	virtual ~CRijndael();
public:
	typedef unsigned char  Passward[32];
	char *GetName(void);

	void FillRand(char *buf, const int len);

	unsigned char* AesDecrypt(unsigned char* pInData,const int len,unsigned char *pStrKey,
		const int keyLen = 256);
	unsigned char* AesEncrypt(unsigned char* pInData,const int len,int& outLen,
						  unsigned char *pStrKey,const int keyLen = 256);
	static int Encrypt(const unsigned char* pInData, int iInDataLen, unsigned char* pOutData, const unsigned char* pKey, int iKeyLen = 128);
	static void Decrypt(const unsigned char* pInData, int iInDataLen, unsigned char* pOutData, const unsigned char* pKey, int iKeyLen = 128);
private:
	...
	void Cycles(volatile boost::uint64_t * rtn);
	u4byte  k_len;
	u4byte  e_key[64];
	u4byte  d_key[64];
